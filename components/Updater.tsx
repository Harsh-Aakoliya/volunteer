...
export function Updater({ version, onProgress, onDone }: { version: string; onProgress: (progress: number) => void; onDone: () => void }) {
  useEffect(() => {
    if (version) {
      updateApk();
    }
  ...
  async function updateApk() {
    ...
    try {
      ...
      alert(`Error during installing APK: ${error instanceof Error ? error.message : 'Unknown error'}`);
      if (onDone) onDone();
