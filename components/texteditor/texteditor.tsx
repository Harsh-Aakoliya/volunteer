...
// Create a forwarded ref component for RichEditor with error handling
const ForwardedRichEditor = forwardRef<RichEditor, any>((props, ref) => {
  try {
    return <RichEditor {...props} ref={ref} />;
  } catch (error) {
    console.error("Error rendering RichEditor:", error);
    return null;
  }
});
...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ 
  ...
  useEffect(() => {
    if (richText.current && initialContent && !draftContent && isEditorReady) {
  ...
  }, [richText.current, initialContent, isEditorReady]);
  ...
  // Failsafe: If editor doesn't initialize within 3 seconds, set it as ready
  useEffect(() => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView className="flex-1 px-4 py-4">
        ...
        {        /* Editor */}
        {/* Error boundary wrapper for the editor */}
        {!isEditorReady && (
        ...
        {isEditorReady === false && (
          <View className="min-h-64 border border-gray-300 border-t-0 rounded-b-lg p-2 bg-red-50 flex items-center justify-center">
            <Text className="text-red-500">Editor failed to load. Please restart the app.</Text>
          </View>
        )}
        {isEditorReady === true && (
          <StyledRichEditor
            ...
            onEditorInitialized={() => {
              try {
                ...
                // Set content here as an additional safety measure with more defensive programming
                setTimeout(() => {
